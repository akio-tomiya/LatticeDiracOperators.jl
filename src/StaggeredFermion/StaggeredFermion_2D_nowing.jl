...
struct StaggeredFermion_2D_nowing{NC} <: AbstractFermionfields_2D{NC}
    NC::Int64
    NX::Int64
    #NY::Int64
    #NZ::Int64
    ...
    fshifted::Array{ComplexF64,4}


    function StaggeredFermion_2D_nowing(NC, NX, NT)
        NG = 1
        NDW = 0
        NY = 1
        NZ = 1
        ...
        return new{NC}(NC, NX, NT, NDW, NG, NV, f, Dirac_operator, fshifted)
    end
end

function Base.size(x::StaggeredFermion_2D_nowing{NC}) where {NC}
    return (x.NC, x.NX, x.NT, x.NG)
    #return (x.NV,)
end

function Base.length(x::StaggeredFermion_2D_nowing{NC}) where {NC}
    return NC * x.NX * x.NY * x.NZ * x.NT * x.NG
end

function Base.similar(x::T) where {T<:StaggeredFermion_2D_nowing}
    return StaggeredFermion_2D_nowing(x.NC, x.NX, x.NT)
end
...
function Dx!(
    ...
    temps::Array{T,1},
    boundarycondition,
) where {T<:StaggeredFermion_2D_nowing,G<:AbstractGaugefields}
    #temp = temps[4]
    temp1 = temps[1]
    temp2 = temps[2]
...
function clear_fermion!(x::StaggeredFermion_2D_nowing{NC}, evensite) where {NC}
...
function shift_fermion(F::StaggeredFermion_2D_nowing{NC}, ν::T) where {T<:Integer,NC}
    if ν == 1
        shift = (1, 0)
    elseif ν == 2
        shift = (0, 1)
    ...
    return Shifted_fermionfields_2D_nowing(F, shift)
end
...
function shift_fermion(
    F::TF,
    shift::NTuple{Dim,T},
) where {Dim,T<:Integer,TF<:StaggeredFermion_2D_nowing}
    return Shifted_fermionfields_2D_nowing(F, shift)
end


function shifted_fermion!(
    x::StaggeredFermion_2D_nowing{NC},
    boundarycondition,
    shift,
) where {NC}
    NX = x.NX
...
function set_wing_fermion!(a::StaggeredFermion_2D_nowing{NC}, boundarycondition) where {NC}
    return
end
