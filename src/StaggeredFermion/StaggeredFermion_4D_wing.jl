...
struct StaggeredFermion_4D_wing{NC} <: AbstractFermionfields_4D{NC}
    ...
    function StaggeredFermion_4D_wing(NC,NX,NY,NZ,NT) 
        NG = 1
        NDW = 1
...
function Dx!(xout::T,U::Array{G,1},
    x::T,temps::Array{T,1},boundarycondition) where  {T <: StaggeredFermion_4D_wing,G <:AbstractGaugefields}
    #temp = temps[4]
    temp1 = temps[1]
    temp2 = temps[2]
...
function set_wing_fermion!(a::StaggeredFermion_4D_wing{NC},boundarycondition) where NC 
    NT = a.NT
    NZ = a.NZ
    NY = a.NY
    NX = a.NX
