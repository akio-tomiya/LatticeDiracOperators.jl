...
include("./MobiusDomainwallFermion_MPILattice_compat.jl")
...
function MobiusDomainwall_Dirac_operator(
    ...
    improved_gpu = check_parameters(parameters, "improved gpu", false)
    use_lattice_matrices = x isa MobiusDomainwallFermion_5D_MPILattice

    if improved_gpu && !use_lattice_matrices
        throw(ArgumentError(
            "\"improved gpu\" requires the LatticeMatrices-backed " *
            "MobiusDomainwallFermion_5D_MPILattice field"))
    end

    if use_lattice_matrices
...
function Domainwall_Dirac_operator(
    ...
    shamir_parameters = copy(parameters)
    shamir_parameters["b"] = 1.0
    shamir_parameters["c"] = 1.0
    return MobiusDomainwall_Dirac_operator(U, x, shamir_parameters)
end
