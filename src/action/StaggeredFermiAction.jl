...
struct StaggeredFermiAction{Dim,Dirac,fermion,gauge,Nf} <: FermiAction{Dim,Dirac,fermion,gauge}
    ...
    function StaggeredFermiAction(D::Dirac_operator{Dim},hascovnet,covneuralnet,parameters_action) where Dim
        ...
        return new{Dim,typeof(D),xtype,Utype,Nf}(hascovnet,covneuralnet,D,Nf,rhmc_info_for_action,rhmc_info_for_MD,
                        _temporary_fermionfields,
                        _temporary_gaugefields)
    end
end

function evaluate_FermiAction(fermi_action::StaggeredFermiAction{Dim,Dirac,fermion,gauge,Nf},U,ϕ::AbstractFermionfields) where {Dim,Dirac,fermion,gauge,Nf}
    ...
    shiftedcg(vec_x,vec_β,x,WdagW,ϕ,eps = W.eps_CG,maxsteps= W.MaxCGstep, verbose = W.verbose_print)
    clear_fermion!(x)
    add_fermion!(x,α0,ϕ)
    for j=1:N
        αk = vec_α[j]
...
function evaluate_FermiAction(fermi_action::StaggeredFermiAction{Dim,Dirac,fermion,gauge,8},U,ϕ::AbstractFermionfields) where {Dim,Dirac,fermion,gauge}
    W = fermi_action.diracoperator(U)
...
function evaluate_FermiAction(fermi_action::StaggeredFermiAction{Dim,Dirac,fermion,gauge,4},U,ϕ::AbstractFermionfields) where {Dim,Dirac,fermion,gauge}
    W = fermi_action.diracoperator(U)
    η = fermi_action._temporary_fermionfields[1]
    solve_DinvX!(η,W',ϕ)
    Sf = dot(η,η)
...
function gauss_sampling_in_action!(η::AbstractFermionfields,U,fermi_action::StaggeredFermiAction{Dim,Dirac,fermion,gauge,Nf}) where {Dim,Dirac,fermion,gauge,Nf}
    gauss_distribution_fermion!(η)
end

function gauss_sampling_in_action!(η::AbstractFermionfields,U,fermi_action::StaggeredFermiAction{Dim,Dirac,fermion,gauge,4}) where {Dim,Dirac,fermion,gauge}
    evensite = false
    W = fermi_action.diracoperator(U)
...
function sample_pseudofermions!(ϕ::AbstractFermionfields,U,fermi_action::StaggeredFermiAction{Dim,Dirac,fermion,gauge,8},ξ::AbstractFermionfields) where {Dim,Dirac,fermion,gauge}
    W = fermi_action.diracoperator(U)
    mul!(ϕ,W',ξ)
    set_wing_fermion!(ϕ)
end

function sample_pseudofermions!(ϕ::AbstractFermionfields,U,fermi_action::StaggeredFermiAction{Dim,Dirac,fermion,gauge,4},ξ::AbstractFermionfields) where {Dim,Dirac,fermion,gauge}
    W = fermi_action.diracoperator(U)
    mul!(ϕ,W',ξ)
    set_wing_fermion!(ϕ)
end
...
function sample_pseudofermions!(ϕ::AbstractFermionfields,U,fermi_action::StaggeredFermiAction{Dim,Dirac,fermion,gauge,Nf},ξ::AbstractFermionfields) where {Dim,Dirac,Nf,fermion,gauge}
    W = fermi_action.diracoperator(U)
    WdagW = DdagD_Staggered_operator(W)
    ...
    shiftedcg(vec_x,vec_β,x,WdagW,ξ,eps = W.eps_CG,maxsteps= W.MaxCGstep, verbose = W.verbose_print)
    clear_fermion!(ϕ)
    add_fermion!(ϕ,α0,ξ)
    for j=1:N
        αk = vec_α[j]
...
function calc_UdSfdU!(UdSfdU::Vector{<: AbstractGaugefields},fermi_action::StaggeredFermiAction{Dim,Dirac,fermion,gauge,Nf},U::Vector{<: AbstractGaugefields},ϕ::AbstractFermionfields) where {Dim,Dirac,fermion,Nf,gauge}
    @assert Nf != 4 && Nf != 8 
    clear_U!(UdSfdU)
...
function calc_UdSfdU!(UdSfdU::Vector{<: AbstractGaugefields},fermi_action::StaggeredFermiAction{Dim,Dirac,fermion,gauge,4},U::Vector{<: AbstractGaugefields},ϕ::AbstractFermionfields) where {Dim,Dirac,fermion,gauge}
...
function calc_UdSfdU!(UdSfdU::Vector{<: AbstractGaugefields},fermi_action::StaggeredFermiAction{Dim,Dirac,fermion,gauge,8},U::Vector{<: AbstractGaugefields},ϕ::AbstractFermionfields) where {Dim,Dirac,fermion,gauge}
    calc_UdSfdU_Nf_4_8!(UdSfdU,fermi_action,U,ϕ)
end

function calc_UdSfdU_Nf_4_8!(UdSfdU::Vector{<: AbstractGaugefields},fermi_action::StaggeredFermiAction{Dim,Dirac,fermion,gauge,Nf},U::Vector{<: AbstractGaugefields},ϕ::AbstractFermionfields) where {Dim,Dirac,fermion,gauge,Nf}
    W = fermi_action.diracoperator(U)
    WdagW = DdagD_Staggered_operator(W)
...
function calc_UdSfdU_fromX!(UdSfdU::Vector{<: AbstractGaugefields},Y,fermi_action::StaggeredFermiAction{Dim,Dirac,fermion,gauge,Nf},U,X;coeff = 1) where {Dim,Dirac,fermion,gauge,Nf}
    W = fermi_action.diracoperator(U)
    mul!(Y,W,X)
