...
"""
    SmearedFermiAction(fermi_action, pseudofermion, smearing)

Evaluate a fermion action on analytically smeared links and pull its force
back. APE and HYP must select `projection=:polar`; their standard
`:max_retr` projection is forward-only. The wrapper replaces every link seen
by the fermion action, corresponding to the all-link/FLOC choice.
"""
struct SmearedFermiAction{
    F<:FermiAction,
    P,
    S<:NativeLinkSmearing,
}
    fermi_action::F
    pseudofermion::P
    smearing::S

    function SmearedFermiAction(
        fermi_action::F,
        pseudofermion::P,
        smearing::S,
    ) where {F<:FermiAction,P,S<:NativeLinkSmearing}
        if !has_smearing_pullback(smearing)
            name = nameof(typeof(smearing))
            message = if smearing isa Union{APESmearing,HYPSmearing}
                projection = repr(smearing.parameters.projection)
                "$name pullback is unavailable for projection=$projection; " *
                "use $name(...; projection=:polar) to enable the analytic " *
                "pullback required by molecular dynamics"
            else
                "$name does not provide the pullback required by " *
                "molecular dynamics"
            end
            throw(ArgumentError(message))
        end
        return new{F,P,S}(fermi_action, pseudofermion, smearing)
    end
end

struct _SmearedFermiActionMDWorkspace{V,C,T}
    smeared::V
    smearing_cache::C
    smeared_link_force::V
    smeared_cotangent::V
...
function md_action_workspace(action::SmearedFermiAction, U)
    return _SmearedFermiActionMDWorkspace(
        similar(U),
...
function md_potential(
    ...
    U,
    workspace::_SmearedFermiActionMDWorkspace,
)
...
"""Refresh pseudofermions using the smeared links owned by the workspace."""
function refresh_smeared_pseudofermions!(
    gaussian,
    action::SmearedFermiAction,
    U,
    workspace::_SmearedFermiActionMDWorkspace,
...
function refresh_smeared_pseudofermions!(
    ...
    action = driver.action
    action isa SmearedFermiAction || throw(ArgumentError(
        "the MDDriver action must be a SmearedFermiAction",
    ))
    return refresh_smeared_pseudofermions!(
        gaussian,
        action,
        U,
        driver.action_workspace,
...
function refresh_smeared_pseudofermions!(
    ...
    action = getproperty(driver.action.terms, name)
    action isa SmearedFermiAction || throw(ArgumentError(
        "the action named $name must be a SmearedFermiAction",
    ))
    workspace = getproperty(driver.action_workspace.terms, name)
    return refresh_smeared_pseudofermions!(gaussian, action, U, workspace)
end
