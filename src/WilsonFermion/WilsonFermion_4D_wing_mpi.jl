...
function setindex_global!(x::T, v, i1, i2, i3, i4, i5, i6) where {T<:WilsonFermion_4D_mpi}
    @error("setindex_global! is not supported in type $(typeof(x))")
end
                ...
                for iy = 1:PN[2]
                    for id = 1:NDW
                        ...
                        for k2 = 1:NC
                            ...
                            send_mesg1[count] =
                                phase *
                                getvalue(a, k2, PN[1] + (id - NDW), iy, iz, it, ialpha)
                ...
                for iy = 1:PN[2]
                    for id = 1:NDW
                        ...
                        for k2 = 1:NC
                            ...
                            count += 1
                            send_mesg2[count] =
                                phase * getvalue(a, k2, id, iy, iz, it, ialpha)
                            #end
                        end
                    end
                end
                ...
                for ix = -NDW+1:PN[1]+NDW
                    for id = 1:NDW
                        ...
                        for k1 = 1:NC
                            ...
                            send_mesg1[count] =
                                phase *
                                getvalue(a, k1, ix, PN[2] + (id - NDW), iz, it, ialpha)
                ...
                for ix = -NDW+1:PN[1]+NDW
                    for id = 1:NDW
                        ...
                        for k1 = 1:NC
                            ...
                            count += 1
                            send_mesg2[count] =
                                phase * getvalue(a, k1, ix, id, iz, it, ialpha)
                ...
                for iy = -NDW+1:PN[2]+NDW
                    for ix = -NDW+1:PN[1]+NDW
                        for k1 = 1:NC
                            ...
                            send_mesg1[count] =
                                phase1 *
                                getvalue(a, k1, ix, iy, PN[3] + (id - NDW), it, ialpha)
                            send_mesg2[count] =
                                phase2 * getvalue(a, k1, ix, iy, id, it, ialpha)
                ...
                for iy = -NDW+1:PN[2]+NDW
                    for ix = -NDW+1:PN[1]+NDW
                        for k1 = 1:NC
                            ...
                            send_mesg1[count] =
                                phase1 *
                                getvalue(a, k1, ix, iy, iz, PN[4] + (id - NDW), ialpha)
                            send_mesg2[count] =
                                phase2 * getvalue(a, k1, ix, iy, iz, id, ialpha)
...
function Wx!(
    ...
    x::T,
    A,
) where {T<:WilsonFermion_4D_mpi,G<:AbstractGaugefields}
...
function add_fermion!(
    c::WilsonFermion_4D_mpi{NC,NDW},
    α::Number,
    a::T1,
    β::Number,
    b::T2,
) where {NC,T1<:Abstractfermion,T2<:Abstractfermion,NDW}#c += alpha*a + beta*b
            ...
            for iz = 1:c.PN[3]
                for iy = 1:c.PN[2]
                    for ix = 1:c.PN[1]
                        @simd for k1 = 1:NC
                            v =
                                getvalue(c, k1, ix, iy, iz, it, ialpha) +
                                α * getvalue(a, k1, ix, iy, iz, it, ialpha) +
                                β * getvalue(b, k1, ix, iy, iz, it, ialpha)
...
function add_fermion!(
    ...
    α::Number,
    a::T1,
) where {NC,T1<:Abstractfermion,NDW}#c += alpha*a 
    #n1,n2,n3,n4,n5,n6 = size(c.f)
...
function WWx!(
    ...
    x::T,
    A,
) where {T<:WilsonFermion_4D_mpi,G<:AbstractGaugefields} #(1 - K^2 Teo Toe) xe
    iseven = true
    temp = A._temporary_fermi[4]#temps[4]
...
function LinearAlgebra.mul!(
    y::WilsonFermion_4D_mpi{3,NDW},
    A::T,
    x::T3,
) where {T<:Abstractfields,T3<:Abstractfermion,NDW}
...
function LinearAlgebra.mul!(
    u::T1,
    x::Abstractfermion,
    y::Adjoint_fermionfields{<:WilsonFermion_4D_mpi{NC,NDW}},
) where {T1<:AbstractGaugefields,NC,NDW}
                        ...
                        for ib = 1:NC
                            @simd for ia = 1:NC
                                v =
                                    ...
                                    getvalue(x, ia, ix, iy, iz, it, ik) *
                                    getvalue(y, ib, ix, iy, iz, it, ik)
...
function LinearAlgebra.mul!(
    ...
    A::T,
    x::T3,
) where {NC,T<:Number,T3<:Abstractfermion,NDW}
...
function LinearAlgebra.mul!(
    xout::WilsonFermion_4D_mpi{NC,NDW},
    A::TA,
    x::WilsonFermion_4D_mpi{NC},
) where {TA<:AbstractMatrix,NC,NDW}
    NX = x.NX
    NY = x.NY
    NZ = x.NZ
