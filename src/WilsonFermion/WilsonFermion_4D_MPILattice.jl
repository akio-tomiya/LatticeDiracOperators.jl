...
import LatticeMatrices: mulT!,
    Shifted_Lattice, LatticeMatrix, Adjoint_Lattice, Oneγ
...
function mul_1plusγμx!(y::WilsonFermion_4D_MPILattice{NC,NX,NY,NZ,NT,T,AT,NDW,NG},
    ...
    if μ == 5
        substitute!(y.f, x.f)
        mul!(y.f, Oneγμ{:plus,5}())
    else
        mul!(y.f, Oneγ(μ, true), x.f)
    end
end
...
function mul_1minusγμx!(y::WilsonFermion_4D_MPILattice{NC,NX,NY,NZ,NT,T,AT,NDW,NG},
    ...
    if μ == 5
        substitute!(y.f, x.f)
        mul!(y.f, Oneγμ{:minus,5}())
    else
        mul!(y.f, Oneγ(μ, false), x.f)
    end
end
...
function Wdagx_noclover_ν!(temp, x, ν, Uν, Ahoppν, Ahopmν, temp1, temp2)
    ...
    mul!(temp1.f, Oneγ(ν, true), temp1.f)
    ...
    mul!(temp2.f, Oneγ(ν, false), temp2.f)
...
function LinearAlgebra.mul!(C::MMatrix{NC,NG,T}, ::Oneγ{1,1}) where {NC,NG,T}
...
function LinearAlgebra.mul!(C::MMatrix{NC,NG,T}, ::Oneγ{1,2}) where {NC,NG,T}
    @inbounds for ic = 1:NC
        x1 = C[ic, 1]
        x2 = C[ic, 2]
        x3 = C[ic, 3]
...
function LinearAlgebra.mul!(C::MMatrix{NC,NG,T}, ::Oneγ{1,3}) where {NC,NG,T}
    @inbounds for ic = 1:NC
        x1 = C[ic, 1]
        x2 = C[ic, 2]
        x3 = C[ic, 3]
...
function LinearAlgebra.mul!(C::MMatrix{NC,NG,T}, ::Oneγ{1,4}) where {NC,NG,T}
    @inbounds for ic = 1:NC
        x1 = C[ic, 1]
        x2 = C[ic, 2]
        x3 = C[ic, 3]
