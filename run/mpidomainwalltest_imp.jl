...
println("Test for 1/D for Domainwall MPI")
...
function test1()
    ...
    L5 = 4
    x = Initialize_pseudofermion_fields(U[1], "Domainwall", L5=L5,nowing = nowing)
    ...
    if mpi
        if x.myrank == 0
            setvalue!(x,10.0,1,1,1,1,1,1,1)
            setvalue!(x,im,1,2,1,1,1,1,1)
            setvalue!(x,2im,1,3,1,1,1,1,1)
            setvalue!(x,3im,1,4,1,1,1,1,1)
            setvalue!(x,10.0,1,1,2,1,1,1,1)
            setvalue!(x,im,1,2,1,1,4,1,1)
            setvalue!(x,2im,1,3,1,4,1,1,1)
            setvalue!(x,3im,1,4,1,1,1,1,1)
        end
    ...
    if mpi
        if x.myrank == 0
            println(getvalue(x,1,1,1,1,1,1,1))
            println(getvalue(x,1,2,1,1,1,1,1))
        end
    else
