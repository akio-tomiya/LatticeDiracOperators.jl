...
function test1()
    ...
    #@info Nx,Ny,Nz,Nt,L5
    println("new")
    stats = @timed  mul!(y, D, x)
    #t += stats.time
    println(dot(y, y))
    if docpu
        println("cpu")
        stats = @timed mul!(ycpu, Dcpu, xcpu)
        #tcpu += stats.time
    ...
    println("Nx,Ny,Nz,Nt,L5 = ", NX, ",", NY, ",", NZ, ",", NT, ",", L5)
    println("Average GPU time: ", t / nt, " sec")
    println("Average CPU time: ", tcpu / nt, " sec")
    return
