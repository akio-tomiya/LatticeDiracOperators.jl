...
# import LatticeDiracOperators: calc_mres_and_derivative_gpu
using CUDA
...
function MDstep!(gauge_action, U, p, MDsteps, Dim, Uold, fermi_action, η, ξ,
    ...
    mres, dmdbs, dmdcs = LatticeDiracOperators.Dirac_operators.calc_mres_and_derivative_gpu(fermi_action, U, 10)
    println("mres = $mres")
...
function test1()
    ...
    #is5D = false
    is5D = true
    x = Initialize_pseudofermion_fields(U[1], "GeneralizedDomainwall"; L5, is5D)
    # x = Initialize_pseudofermion_fields(U[1], "MobiusDomainwall"; L5, is5D, PEs=(1,1,1,4))

    xcpu = Initialize_pseudofermion_fields(Ucpu[1], "GeneralizedDomainwall"; L5)
    ...
    # params["b"] = 2.14
    # params["c"] = 2.39
    params["bs"] = fill(1.5, L5)
    params["cs"] = fill(0.5, L5)
    params["M"] = -1.9
    #params["improved gpu"] = true
